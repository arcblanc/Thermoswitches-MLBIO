#!/usr/bin/env bash
# CPU Spot smoke — validate GCS sync + pipeline wiring (2 sequences, no GPU).
set -euo pipefail

LOG=/var/log/thermoswitches-cpu-smoke.log
exec > >(tee -a "$LOG") 2>&1

echo "=== Thermoswitches CPU smoke $(date -Is) ==="
...
echo "=== CPU smoke complete $(date -Is) ==="
