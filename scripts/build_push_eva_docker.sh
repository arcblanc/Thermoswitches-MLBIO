...
#
# Usage (on the VM):
#   bash scripts/build_push_eva_docker.sh           # build only
#   bash scripts/build_push_eva_docker.sh --push    # build + docker push
#
# Env (optional):
...
CUDA + flash-attn layers must be built on a temporary Linux x86_64 VM:
  ...
  3. On the VM:  bash scripts/build_push_eva_docker.sh --push
