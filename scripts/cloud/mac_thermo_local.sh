...
# Usage:
#   bash scripts/cloud/mac_thermo_local.sh smoke   # 4 sequences end-to-end
#   bash scripts/cloud/mac_thermo_local.sh full    # 2396 train + 10000 de novo
set -euo pipefail
...
PY
  ...
  full)
    ...
    ;;
  *)
    echo "Usage: bash scripts/cloud/mac_thermo_local.sh [smoke|full]"
    exit 1
    ;;
esac
