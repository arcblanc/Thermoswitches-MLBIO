...
def _best_e_rfam(blast_e: object, nhmmer_e: object) -> float:
    ...
    values = []
    if blast_e is not None and not (isinstance(blast_e, float) and math.isnan(blast_e)):
        try:
            if pd.notna(blast_e):
                values.append(float(blast_e))
        except (TypeError, ValueError):
            pass
    if nhmmer_e is not None and not (
        isinstance(nhmmer_e, float) and math.isnan(nhmmer_e)
    ):
        try:
            if pd.notna(nhmmer_e):
                values.append(float(nhmmer_e))
        except (TypeError, ValueError):
            pass
    if not values:
        return float("inf")
...
def _gate_counts(
    ...
    def _novel(e: object) -> bool:
        ...
        if isinstance(e, float) and math.isinf(e):
            return True
        try:
            return float(e) > evalue_novelty_min
        except (TypeError, ValueError):
            return False
