...
def _as_int(value: object, default: int = 0) -> int:
    """Coerce JSON/state scalars to int for counters and gate tallies."""
    if value is None:
        return default
...
def _as_float(value: object, default: float = 0.0) -> float:
    """Coerce JSON/state scalars to float for yield ratios."""
    if value is None:
        return default
    if isinstance(value, bool):
...
def _as_dict_list(value: object) -> list[dict[str, object]]:
    """Return only mapping entries from a JSON list payload."""
    if not isinstance(value, list):
        return []
    return [item for item in value if isinstance(item, dict)]


def _chunk_summary(summary: dict[str, object]) -> dict[str, object]:
    """Normalize yield sidecars that nest stats under a `chunk` key."""
...
def _import_compute_yield_ratio():
    """Import eva_yield_ratio from the triage scripts directory."""
    triage_dir = ROOT / "scripts" / "triage"
    if str(triage_dir) not in sys.path:
        sys.path.insert(0, str(triage_dir))
    from eva_yield_ratio import compute_yield_ratio

    return compute_yield_ratio
...
def _run_yield(
    *,
    ...
    compute_yield_ratio = _import_compute_yield_ratio()
    return compute_yield_ratio(
        dynamic_csv=dynamic_csv,
        blast_tsv=blast_tsv,
        nhmmer_tbl=nhmmer_tbl,
        output_csv=output_csv,
        output_json=output_json,
        chunk_id=chunk_id,
        output_chunk_json=output_chunk_json,
...
def analyze_and_log_slice(
    *,
    ...
    cats = _as_dict(novelty.get("categories"))
    n = _as_int(chunk_summary.get("n"), _as_int(cum.get("n_quality_gated")))
    n_pass = _as_int(chunk_summary.get("n_pass"))
    y = _as_float(chunk_summary.get("yield_ratio"))
    rolling_n = _as_int(cum.get("n_quality_gated"))
    rolling_pass = _as_int(cum.get("n_yield_pass"))
    rolling_y = _as_float(cum.get("rolling_yield"), _as_float(cum.get("yield_ratio")))
    ...
    analysis = {
        ...
        "gates": {
            "z_le_m2": _as_int(chunk_summary.get("z_le_m2")),
            "dp_gt_0": _as_int(chunk_summary.get("dp_gt_0")),
            "novel": _as_int(chunk_summary.get("novel")),
        },
        "novelty": {
        ...
        "rolling": {
            ...
            "chunks": _as_dict_list(cum.get("chunks")),
        },
    ...
    near = _as_dict(cats.get("identical_near"))
    remote = _as_dict(cats.get("remote_homolog"))
    no_hit = _as_dict(cats.get("no_hit"))
...
def backfill_analysis(args: argparse.Namespace) -> None:
    ...
    stream_root = resolve_path(STREAM_ROOT)
    chunks = _as_dict_list(state.get("chunks"))
    if not chunks:
    ...
    for chunk in chunks:
        ...
        rolling: dict[str, object] = {
            "n_quality_gated": sum(_as_int(c.get("n")) for c in cum_chunks),
            "n_yield_pass": sum(_as_int(c.get("n_pass")) for c in cum_chunks),
            "yield_ratio": (
                sum(_as_int(c.get("n_pass")) for c in cum_chunks)
                / max(1, sum(_as_int(c.get("n")) for c in cum_chunks))
            ),
            "rolling_yield": (
                sum(_as_int(c.get("n_pass")) for c in cum_chunks)
                / max(1, sum(_as_int(c.get("n")) for c in cum_chunks))
...
def triage_once(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        state["consecutive_source_failures"] = (
            _as_int(state.get("consecutive_source_failures")) + 1
        )
    ...
    last = _as_int(state.get("last_triaged_count"), args.seed_count)
    ...
    chunk_num = len(_as_dict_list(state.get("chunks"))) + 1
    ...
    prior = _as_dict_list(state.get("chunks"))
    prior.append(_chunk_summary(chunk_summary))
    cum = _run_yield(
    ...
    print(
        ...
        f"= {_as_float(cum.get('yield_ratio')):.4f}"
    )

    analyze_and_log_slice(
        slice_id=slice_id,
        stream_root=stream_root,
        chunk_summary=_chunk_summary(chunk_summary),
        cum=cum,
