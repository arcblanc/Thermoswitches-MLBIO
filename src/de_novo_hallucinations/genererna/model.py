...
class LayerNorm(nn.Module):
    def __init__(self, ndim: int, bias: bool) -> None:
        """Initialize learnable LayerNorm scale and optional bias."""
        super().__init__()
...
class CausalSelfAttention(nn.Module):
    def __init__(self, config: "GPTConfig") -> None:
        """Build causal multi-head attention with optional flash attention."""
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Compute causal self-attention over the input embeddings."""
        B, T, C = (
            x.size()
...
class MLP(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply GELU feed-forward projection with residual dropout."""
        x = self.c_fc(x)
        x = self.gelu(x)
        x = self.c_proj(x)
        x = self.dropout(x)
...
class Block(nn.Module):
    def __init__(self, config: "GPTConfig") -> None:
        """Build one transformer block of attention plus MLP."""
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply residual attention and MLP sublayers."""
...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig) -> None:
        """Initialize GPT weights, embeddings, and the language-model head."""
        super().__init__()
    ...
    def _init_weights(self, module: nn.Module) -> None:
        """Initialize Linear and Embedding weights with the GPT-2 scheme."""
    ...
    def crop_block_size(self, block_size: int) -> None:
        """Shrink positional embeddings and causal masks to a smaller block size."""
    ...
    def generate(
        ...
        """Autoregressively generate token ids from a prompt tensor."""
        ...
        if strategy == "beam_search":
            ...
            for _ in range(max_new_tokens):
                ...
                for i in range(beam_size):
                    idx_cond = (
                        beam_seqs[i]
                        if beam_seqs[i].size(1) <= self.config.block_size
                        else beam_seqs[i][:, -self.config.block_size :]
                    )
                    ...
                    for j in range(beam_size):
                        candidate_seq = torch.cat(
                            [beam_seqs[i], indices[:, j : j + 1]], dim=1
                        )
                ...
                all_candidates.sort(
                    key=lambda x: x[0].mean().item() + torch.rand(1).item() * 5e-1,
                    reverse=True,
                )

                beam_seqs = [
                    all_candidates[i][1]
                    for i in range(min(beam_size, len(all_candidates)))
                ]
            ...
            if not completed_seqs:
                completed_seqs = [
                    (beam_scores[:, i], beam_seqs[i]) for i in range(beam_size)
                ]
