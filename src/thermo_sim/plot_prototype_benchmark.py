...
def load_benchmark_report(path: str = DEFAULT_REPORT) -> dict:
    """Load the prototype benchmark JSON report."""
    with resolve_path(path).open() as handle:
        return json.load(handle)


def load_fused_features(path: str = DEFAULT_FUSED) -> pd.DataFrame:
    """Load the prototype fused-features CSV."""
...
def save_figure(fig: plt.Figure, output_dir: Path, name: str) -> Path:
    """Save a matplotlib figure to output_dir and close it."""
...
def _sequence_rows(report: dict) -> pd.DataFrame:
    """Build a per-sequence length, runtime, and memory table from the report."""
    rows = []
    ...
    for seq in report.get("sequences", []):
        ...
        nupack_mem = (
            seq.get("nupack", {}).get("memory_mb", {}).get("rss_after_mb", vienna_mem)
        )
...
def plot_length_vs_cost(report: dict, output_dir: Path) -> Path:
    """Plot sequence length against peak RSS and wall time."""
...
def plot_categorical_bars(report: dict, output_dir: Path) -> Path:
    """Plot CPU utilization and Vienna dangle-model RMSE bars."""
    ...
    if dangle_cmp:
        ...
        axes[1].text(
            0.5, 0.5, "dangle_comparison not in report", ha="center", va="center"
        )
        axes[1].set_axis_off()
...
def plot_feature_violins(fused_df: pd.DataFrame, output_dir: Path) -> Path:
    """Plot Vienna vs NUPACK feature distributions as violin plots."""
    records = []
    ...
    for ax, feature_name in zip(axes, [f[0] for f in FEATURE_PAIRS]):
        ...
        if subset.empty:
            ...
            continue
        sns.violinplot(
            data=subset, x="engine", y="value", hue="label", ax=ax, inner="box", cut=0
        )
...
def _hill_overlay(
    temps: list[float], values: list[float]
) -> tuple[dict, np.ndarray, np.ndarray]:
    """Fit a Hill curve and return the fit, dense temperatures, and overlay."""
...
def plot_melting_profiles(report: dict, output_dir: Path) -> Path:
    """Plot SD-window melting scatter with a generalised Hill overlay."""
        ...
        for col_idx, engine in enumerate(("vienna", "nupack")):
            ...
            if values:
                ...
                if np.all(np.isfinite(curve)):
                    ax.plot(
                        dense_t, curve, color="crimson", linewidth=2, label="Hill fit"
                    )
                tm = fit.get("Tm", tm)
...
def _build_parser() -> argparse.ArgumentParser:
    """Build the prototype figure-generation CLI parser."""
    parser = argparse.ArgumentParser(
        description="Generate prototype benchmark visualization figures."
    )
...
def main() -> None:
    """Generate prototype benchmark figures from the CLI."""
